...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument("--completion-model", type=str, default="deepseek-chat", help="The model to use to generate questions and answers (gpt-3.5, gpt-4, ...)")
...
def get_chunks(
    ...
    if doctype == "api":
        ...
        embeddings = build_langchain_embeddings(openai_api_key= 'xxxopenai_embedding_modelxxx', model=model)
...
def add_chunk_to_dataset(
    ...
    for q in qs:
        ...
        indices.remove(i)
        for j in np.random.choice(indices, num_distract):
...
def main():
    ...
    # )
    client = OpenAI(api_key="deepseek_model", base_url="https://api.deepseek.com")
    ...
    for k in range(150,200):
        chunks = get_chunks(args.datapath, args.doctype, k, CHUNK_SIZE, OPENAPI_API_KEY, model=args.embedding_model)
        ...
        list_temp=ds.to_list()
        for m in range(0,len(list_temp)):
            list.append(list_temp[m])
        ...
        # list.append(ds['instruction'])
    # Save as .arrow format
    # ds.save_to_disk(args.output)
    
    # Save as .jsonl format
    # list.to_json(args.output + ".jsonl")
    with open('output_f.json', 'w') as file:
        json.dump(list, file, indent=4)
